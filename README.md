...
## Handle All three Relation In Entity Table
 - To create relationships in Entity Framework Core, i primarily define navigation properties between entities. For one-to-one, a navigation property is added on both sides. For one-to-many, a collection navigation property is added on the "one" side. For many-to-many, a junction table is used with navigation properties on both sides. EF Core can often infer relationships based on naming conventions, but you can also use Fluent API for more customization.
...
## Types of Web Services(asp .Net Core)
